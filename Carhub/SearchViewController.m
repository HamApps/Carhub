...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if([pickerView isEqual:enginePicker]){
        ...
        if (row == 3){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"F12"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
            NSLog(@"transmissionarray%lu", (unsigned long)EngineArray1.count);
        }
        ...
        if (row == 5){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"Flat-6"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
        }
        if (row == 6){
        ...
        if (row == 9){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"l6"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
        }
        if (row == 10){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"l8"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
        }
        if (row == 11){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"V6"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
        }
        if (row == 12){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"V8"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
        }
        if (row == 13){
        ...
        if (row == 14){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"V12"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
        }
        if (row == 15){
            EnginePredicate = [NSPredicate predicateWithFormat:@"Engine CONTAINS %@", @"W12"];
            EngineArray1 = [PriceArray1 filteredArrayUsingPredicate:EnginePredicate];
            NSLog(@"pricedata%@", EngineArray1);
        }
        if (row == 16){
...
- (IBAction)UsePredicates {
    ...
    for (int i=0; i < ZerotoSixtyArray1.count; i++)
    {
        //Create our city object
        NSString * cMake = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Make"];
        NSString * cModel = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Model"];
        NSString * cYearsMade = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Years Made"];
        NSString * cPrice = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Price"];
        NSString * cEngine = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Engine"];
        NSString * cTransmission = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Transmission"];
        NSString * cDriveType = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Drive Type"];
        NSString * cHorsepower = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Horsepower"];
        NSString * cZeroToSixty = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"0-60"];
        NSString * cTopSpeed = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Top Speed (mph)"];
        NSString * cWeight = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Weight (lbs)"];
        NSString * cFuelEconomy = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Fuel Economy (mpg)"];
        NSString * cURL = [[ZerotoSixtyArray1 objectAtIndex:i] objectForKey:@"Image URL"];
        
        //Add the city object to our cities array
        [finalArray addObject:[[Model alloc]initWithCarMake:cMake andCarModel:cModel andCarYearsMade:cYearsMade andCarPrice:cPrice andCarEngine:cEngine andCarTransmission:cTransmission andCarDriveType:cDriveType andCarHorsepower:cHorsepower andCarZeroToSixty:cZeroToSixty andCarTopSpeed:cTopSpeed andCarWeight:cWeight andCarFuelEconomy:cFuelEconomy andCarImageURL:cURL]];
        NSLog(@"cararray%@", finalArray);
        NSLog(@"predicatescount%lu", (unsigned long)finalArray.count);
    }
}
